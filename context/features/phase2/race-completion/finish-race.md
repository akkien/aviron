...
- **The unifying condition is "zero live participants remaining"** — `len(participants) == 0` in `room-actor-core.md`'s in-memory map. Everything else in this section is really just "how do we reach that state," not a separate rule. Two distinct paths get there, and **both** must trigger the same finish sequence:
  1. **Everyone who connected eventually left**: `reconnection/grace-period.md` applies a `ParticipantLeft` event per participant as their individual grace periods expire; the moment the one that empties `participants` entirely is applied, that's this feature's finish condition firing. Every `FinishRank`/`FinishTimeMs` stays `NULL` (nobody reached the target), `DisconnectedCount` reflects however many times each of them actually dropped.
  2. **Nobody ever connected in the first place**: the race started (a room actor exists), but no client ever opened the WebSocket and sent `join_race` for it — `participants` is empty from the moment the actor spawns and stays that way. Nothing in path (1) covers this: no `ParticipantDisconnected`, no grace timer, no `ParticipantLeft` ever fires, because none of those exist without at least one prior `ParticipantJoined`. This needs its own trigger — a room-level, one-shot timeout started when the actor is constructed (mirroring the per-participant grace timer's self-scheduling shape, `time.AfterFunc` calling `Send` back onto its own `inbox`), checked once: if nobody has ever joined by the time it fires, that's the same "zero live participants remaining" condition, so it runs the exact same finish sequence. If even one participant joined before the timeout fires, it's a no-op — path (1) or a real finish already governs from that point on.
- Both paths result in identical output: an empty-or-all-`NULL` result set. Broadcasting `race_finished` to a room with zero attached connections is a harmless no-op (the hub has nothing registered to fan out to), not an error worth special-casing.
- **A participant who registered via REST (`POST /races/{id}/join`) but never once connected live is invisible to the room actor** — it never received a `ParticipantJoined` for them, so they never appear in `participants` at all, and the finish transaction (below) only ever iterates `participants`. Their `race_participants` row is simply never touched by this transaction, left at whatever `AddParticipant` set at join time (`finish_rank`/`finish_time_ms` `NULL`, `disconnected_count` `0`) — which is already the semantically correct outcome for "registered but never actually raced," not a gap to special-case.
- **This is also where `room-registry.md`'s Teardown section is actually implemented for the empty-room case.** That spec attributes "removed... when every participant has been disconnected past the grace period with nobody left" to `reconnection/grace-period.md`, but `grace-period.md` itself only ever produces the `ParticipantLeft` event — detecting that it emptied the room (via either path above), running the Postgres transaction, broadcasting `race_finished`, and calling `Registry.Remove`/cancelling the actor's context all happen here, in this feature's finish-condition check.
...
- The room-level "nobody ever joined" timeout (Finish Condition, path 2) follows the exact same self-scheduling shape `reconnection/grace-period.md`'s per-participant timer already established: `time.AfterFunc` calling `r.Send(...)` back onto the actor's own `inbox`, never touching `participants` from the timer's own goroutine. Unlike the per-participant timer, it never needs cancelling — the moment anyone joins, its firing becomes a no-op (checked inside `applyEvent`, not by stopping the timer), so there's no `*time.Timer` reference to track or `Stop()`
