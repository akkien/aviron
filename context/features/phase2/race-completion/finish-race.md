...
- **Abandoned race (edge case, but a real, first-class path through this same finish sequence, not a special case)**: a race where every participant leaves without anyone finishing still needs to end — nobody left to finish it. `reconnection/grace-period.md` applies a `ParticipantLeft` event per participant as their individual grace periods expire; the moment the one that empties `participants` entirely is applied, that *is* this feature's "zero participants remaining active" finish condition firing. Runs the exact same finish sequence below — one transaction, `race_finished` broadcast, actor teardown — just with every participant's `FinishRank`/`FinishTimeMs` left `NULL` (nobody reached the target) and `DisconnectedCount` reflecting however many times each of them actually dropped. Broadcasting `race_finished` to a room with zero attached connections is a harmless no-op (the hub has nothing registered to fan out to), not an error worth special-casing.
- **This is also where `room-registry.md`'s Teardown section is actually implemented for the empty-room case.** That spec attributes "removed... when every participant has been disconnected past the grace period with nobody left" to `reconnection/grace-period.md`, but `grace-period.md` itself only ever produces the `ParticipantLeft` event — detecting that it emptied the room, running the Postgres transaction, broadcasting `race_finished`, and calling `Registry.Remove`/cancelling the actor's context all happen here, in this feature's finish-condition check.
