...
- `participants` is bounded by `internal/race.MaxParticipants` (10) — Phase 1's `JoinRace` enforces this at join time (`context/features/phase1/races/join-race.md`), so a room actor never has more than 10 entries in this map, and `broadcastSnapshot()`'s per-tick payload is bounded accordingly. Nothing in `internal/room` needs to re-check this; it only ever spawns with however many participants already joined before start.
