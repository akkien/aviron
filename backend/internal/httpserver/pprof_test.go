...
func TestPprof_EnabledServesDebugEndpoints(t *testing.T) {
	...
	httpserver.RegisterRoutes(mux, config.Config{PprofEnabled: true}, pool, ctx, room.NewRegistry(testLogger, testTickObserver), testLogger, newTestMetrics())
...
func TestPprof_DisabledReturns404(t *testing.T) {
	...
	mux := httpserver.NewServer()
	httpserver.RegisterRoutes(mux, config.Config{PprofEnabled: false}, pool, ctx, room.NewRegistry(testLogger, testTickObserver), testLogger, newTestMetrics())
	srv := newTestServer(t, mux)
	defer srv.Close()
