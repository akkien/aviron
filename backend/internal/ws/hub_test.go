...
func TestHubRegistry_GetOrCreate_ReturnsSameHubForSameRace(t *testing.T) {
	...
	actor := room.NewRoomActor(ctx, "race-1", 3, make(chan []byte, 4), fakeFinisher{}, fakeLeaver{})
...
func TestHubRegistry_GetOrCreate_DifferentRacesGetDifferentHubs(t *testing.T) {
	...
	actor1 := room.NewRoomActor(ctx, "race-1", 3, make(chan []byte, 4), fakeFinisher{}, fakeLeaver{})
	actor2 := room.NewRoomActor(ctx, "race-2", 3, make(chan []byte, 4), fakeFinisher{}, fakeLeaver{})
