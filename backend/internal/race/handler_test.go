...
func TestRaceHandler_Leave_OK(t *testing.T) {
	secret := []byte("test-secret")
	h := newTestHandler()
	raceID := createTestRace(t, secret, h)
	token := signTestToken(t, secret, "user-2")
	...
	req := httptest.NewRequest(http.MethodPost, "/races/"+raceID+"/leave", nil)
	req.SetPathValue("id", raceID)
	req.Header.Set("Authorization", "Bearer "+token)
	rec := httptest.NewRecorder()

	middleware.Auth(secret)(http.HandlerFunc(h.Leave)).ServeHTTP(rec, req)
...
func TestRaceHandler_Leave_InvalidRaceID(t *testing.T) {
	secret := []byte("test-secret")
	h := newTestHandler()
	token := signTestToken(t, secret, "user-1")

	req := httptest.NewRequest(http.MethodPost, "/races/not-a-uuid/leave", nil)
	req.SetPathValue("id", "not-a-uuid")
	req.Header.Set("Authorization", "Bearer "+token)
	rec := httptest.NewRecorder()
...
func TestRaceHandler_Leave_NotFound(t *testing.T) {
	secret := []byte("test-secret")
	h := newTestHandler()
	token := signTestToken(t, secret, "user-1")
...
func TestRaceHandler_Leave_NotParticipant(t *testing.T) {
	secret := []byte("test-secret")
	h := newTestHandler()
	raceID := createTestRace(t, secret, h)
	token := signTestToken(t, secret, "user-2") // never joined
...
func TestRaceHandler_Leave_NotPending(t *testing.T) {
	secret := []byte("test-secret")
	repo := newFakeRepository()
	svc := race.NewRaceService(repo, secret)
	h := race.NewRaceHandler(svc, room.NewRegistry(), context.Background())
...
func TestRaceHandler_Leave_MissingAuth(t *testing.T) {
	h := newTestHandler()

	req := httptest.NewRequest(http.MethodPost, "/races/some-id/leave", nil)
	req.SetPathValue("id", "some-id")
	// No Authorization header, and not wrapped in middleware.Auth — simulates
	// the handler being invoked directly, to prove its own defensive check.
	rec := httptest.NewRecorder()
