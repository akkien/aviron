...
func newTestHandler() *race.RaceHandler {
	...
	return race.NewRaceHandler(svc, room.NewRegistry(testLogger, testTickObserver, room.NoopLocator{}, room.NoopPublisher{}), context.Background(), testLogger)
}
...
func TestRaceHandler_Join_NotPending(t *testing.T) {
	...
	h := race.NewRaceHandler(svc, room.NewRegistry(testLogger, testTickObserver, room.NoopLocator{}, room.NoopPublisher{}), context.Background(), testLogger)
